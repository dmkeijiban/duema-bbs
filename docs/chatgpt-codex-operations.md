...
- パックには通常カードだけでなく、シークレット、トレジャー、その他の収録バージョンも含め、`pack.card_count` と登録カード総数を一致させる。`section` は `base | secret | treasure | other` を使い、商品ページでは区分ごとに表示する。
- カードは公式ナンバリング順を基本とし、pack slug・card slug・`sort_order` の重複を残さない。既存slug・id・並び順を上書きしない。
- 画像がないカードは画像を捏造せず、既存の `ZukanPseudoCard` で表示する。カード名、コスト、種類、パワー、レアリティ、文明が欠けたり重なったりしないことを確認する。
- 単色カードは現在の文明別フレーム・背景・見出し色を維持し、多色対応のために単色カードの見た目を変えない。
- 多色カードは `civilization` のスラッシュ区切り順をそのまま使い、2〜5色を薄い文明色で縦方向・等幅に分割してカード背景全体へ表示する。色数が増えても同じ方式で分割する。
- 多色カードのカード名部分は半透明の白背景で可読性を確保し、カード名は最大2行、コスト表示を隠さない。カード上端の細い色帯・虹色バーは表示しない。
- 新しいパックのPreviewでは、スマホとPCの両方で単色・2色・3色以上・無色・長いカード名を代表抽出し、一覧カード、区分見出し、ページ送り、カード詳細を目視確認する。既存の `/zukan/dm-01`、`/zukan/dm-02`、`/zukan/dm22-rp1?page=3` も回帰確認する。
