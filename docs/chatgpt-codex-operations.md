...
- パックには通常カードだけでなく、シークレット、トレジャーの収録バージョンも含め、`pack.card_count` と登録カード総数を一致させる。現行で対応する `section` は `base | secret | treasure` のみとし、新しい区分が必要な場合は先に検証・表示コードを対応させてからルールへ追加する。
...
- カード名、カード種別、文明、コスト、マナ、種族、パワー、レアリティ、能力、フレーバー、イラストレーター、公式URLは公式カード詳細を正本とし、不明な値を推測で埋めない。取得できない値は `null` のまま残し、警告内容を確認してから登録する。
- 新しいパックは、全カード・全区分のJSON検証、件数一致、Preview確認が終わるまで公開しない。部分投入状態や一部ページだけ揃った状態を公開せず、公開ページは全ページが揃っていない場合に表示させない。
- `section` とページ範囲を持つパックでは、各カードが必ず1区分・1ページだけに含まれるよう確認し、区分境界で欠落・重複・見出し違いを起こさない。パック固有の範囲設定を追加した場合は、その理由と件数をコードまたは関連ドキュメントへ残す。
- 公開後のカードは、思い出投稿・評価との紐づけを守るため、slug・id・pack_idを変更せず、削除して再登録しない。誤記修正は既存レコードを維持した安全な更新とし、URL変更が避けられない場合はalias・redirectを先に設計する。
- ツインパクト、両面、サイキック、ドラグハート、複数パーツなど特殊構造のカードは、通常クリーチャーの項目へ無理に押し込まない。公式上の1枚の物理カード・各面・各パーツの関係を確認し、カード名、種別、コスト、マナ、種族、パワーが別項目へ混入していないことを個別確認する。
- ツインパクトはクリーチャー面だけでなく呪文面の名前・コスト・能力も公式カード詳細から取得する。JSONの `ability_text` は、クリーチャー面の能力に続けて `【呪文面】呪文名（コスト N）`、改行、呪文面の能力という共通形式で保存する。`zukan:validate` は新規パックでこの形式がないツインパクトをエラーにし、カード詳細は共通パーサーで自動的に両面を分けて表示する。
- ChatGPTで思い出図鑑記事を書くときは、カードの完全URLやカード名ではなく、`/zukan/card/<slug>` の末尾slugを `{{CARD:<slug>}}` で指定する。複数カードは `{{CARDGRID:見出し` の次行からslugを1行ずつ記載し、最大6枚までとする。
- 記事保存時は、`card` / `cardGrid` が参照する全カードslugについて、公開済みカードとして存在することを確認する。存在しない・非公開・重複・7枚以上の指定は保存エラーとし、カードが黙って消える状態を作らない。
- 記事内のカードは、公式画像があれば画像を使い、画像がなければ `ZukanPseudoCard` を表示する。両面カードは同一slugのまま表面画像を代表表示し、カード詳細へのリンク先で表面・裏面を続けて表示する。
- 新しく公開したカードはハードコード一覧へ追加せず、DBの公開済みカードから記事管理候補と記事表示を取得する。件数上限で候補から漏れないようページ分割取得を維持する。
- 記事を公開する前に管理画面Previewで、指定した全カードが入力順どおり表示され、各カードから正しい `/zukan/card/<slug>` へ遷移することを確認する。既存記事のカード位置と本文ブロック順を保存時に変えない。
