...
- 主要カード候補を抽出
- 主要カードの個別Wikiページを取得
- カード表から文明、コスト、カードタイプ、種族、パワー、主要能力を抽出
- 読み物記事のMarkdown下書きを生成
- カードごとに「昔の印象」「今の役割」「現代化された部分」が入るようにする
- 本文内の《カード名》を抽出
- 公式カード検索から画像を取得
- 初出カード名の直後に画像を挿入
...
- 商品記事は8〜12枚の主要カードを紹介します。
- 見出しは「主要カードを見る」のような説明見出しではなく、読み物として内容が伝わる形にします。
- 各カード紹介には、文明、コスト、カードタイプ、種族、パワー、主要能力を入れます。
- 「能力面では〜が文脈になる」「このカード自身が今のゲームで何を担当するかを見たい」のような薄い定型文は品質チェックで落とします。
- 記事本文にはHTMLタグを直接混ぜず、Markdown下書きとして出します。
- 表示側では記事本文が白枠の横幅を自然に使うため、本文だけが不自然に細くなる改行は避けます。
