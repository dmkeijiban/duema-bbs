...
const summary = { mode: execute ? 'execute' : 'dry-run', target, inputPath, faces: faces.length, pages: pages.length, logical_cards: new Set(faces.map((face) => face.card_id)).size }
if (!execute) {
...
for (let index = 0; index < faces.length; index += 200) {
  const { error } = await supabase.from('card_faces').upsert(faces.slice(index, index + 200).map((face) => ({ ...face, updated_at: new Date().toISOString() })), { onConflict: 'card_printing_id,side_index' })
