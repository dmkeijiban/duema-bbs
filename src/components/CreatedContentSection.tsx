...
export function CreatedContentSection({ resume, avatarUrl, resumeUpdatedAtLabel, nine, deck }: {
  ...
  return <section className="rounded border border-gray-200 bg-white">
    ...
    <div className="grid items-stretch gap-3 p-3 md:grid-cols-3">
      <article className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
        ...
        {resume ? <ResumeMypagePanel data={resume.data} avatarUrl={avatarUrl} isPublic={resume.isPublic} updatedAtLabel={resumeUpdatedAtLabel} resumeDate={resume.updatedAt} /> : <div className="flex flex-1 flex-col items-center justify-center py-5 text-center"><p className="text-sm text-slate-500">まだ作成していません</p><Link href="/makers/resume-maker" className={`${actionClass} mt-3`}>作成する</Link></div>}
      </article>
      ...
      <article className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
        ...
        {nine.representative ? <div className="mt-3 flex min-h-0 flex-1 flex-col">
          <Link href={`/makers/my-duema-9/submissions/${nine.representative.id}`} className="grid min-h-0 flex-1 grid-cols-3 grid-rows-3 overflow-hidden rounded-lg border border-slate-200 transition active:scale-[0.99]">
            {nine.representative.items.slice(0, 9).map(item => item.card.image_url ? <img key={`${item.card_id}:${item.position}`} src={item.card.image_url} alt={item.card.name} className="h-full min-h-0 w-full object-cover" /> : <span key={`${item.card_id}:${item.position}`} className="flex min-h-0 items-center justify-center bg-slate-100 p-1 text-center text-[9px]">{item.card.name}</span>)}
          </Link>
          <Link href="/makers/my-duema-9/submissions?tab=mine" className={`${actionClass} mt-3 w-full`}>すべて見る</Link>
        </div> : <div className="flex flex-1 flex-col items-center justify-center py-5 text-center"><p className="text-sm text-slate-500">まだ作成していません</p><Link href="/makers/my-duema-9" className={`${actionClass} mt-3`}>作成する</Link></div>}
      </article>
      ...
      <article className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
        ...
        {deck.items.length > 0 ? <div className="mt-3 flex min-h-0 flex-1 flex-col">
        ...
        </div> : <div className="flex flex-1 flex-col items-center justify-center py-5 text-center"><p className="text-sm text-slate-500">まだ作成していません</p><Link href="/makers/deck-maker" className={`${actionClass} mt-3`}>作成する</Link></div>}
      </article>
    </div>
  </section>
}
