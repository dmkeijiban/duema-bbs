...
const CardCatalogGridItem = memo(function CardCatalogGridItem({ card, index, count, onSelect, selectedBadge, renderCardArt }: {
  card: DeckCard
  index: number
  count: number
  ...
  return <article className="group relative min-w-0 overflow-hidden rounded-md bg-slate-100 ring-1 ring-slate-200 [content-visibility:auto] [contain-intrinsic-size:auto_210px]">
...
export function CardCatalogGrid({ cards, query, loading, hasMore, onLoadMore, onSelect, selectedCount, selectedBadge, renderCardArt }: {
  ...
  return <div ref={scroller} data-testid="search-results" className="grid h-[65dvh] min-h-[360px] max-h-[680px] auto-rows-max content-start grid-cols-4 gap-1.5 overflow-y-auto overscroll-contain p-2.5 sm:gap-2 sm:p-3 lg:h-[calc(100dvh-170px)] lg:max-h-none">
    ...
    {cards.map((card, index) => (
      <CardCatalogGridItem key={cardLogicalKey(card)} card={card} index={index} count={selectedCount?.(card) ?? 0} onSelect={onSelect} selectedBadge={selectedBadge} renderCardArt={renderCardArt} />
    ))}
