...
export type PublicDeckCardData = {
  ...
  user_id: string | null
  created_at: string
  format?: 'original' | 'advance' | string
...
export function PublicDeckCard({ deck, authorName }: { deck: PublicDeckCardData; authorName: string }) {
  ...
  const keyCard = visibleDeckData.find(card => card.id === deck.key_card_id && (!deck.key_card_printing_id || card.printingId === deck.key_card_printing_id))
    ...
    ?? visibleDeckData[0]
  const uniqueCards = visibleDeckData.length
  const totalCards = visibleDeckData.reduce((sum, card) => sum + card.count, 0)
  ...
  return (
    <Link href={`/makers/deck-maker/submissions/${deck.id}`} className="group grid grid-cols-[112px_1fr] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md active:scale-[0.99] sm:grid-cols-[138px_1fr]">
      ...
      <div className="flex min-w-0 flex-col p-4">
        <p className="text-[11px] font-bold text-blue-700">{format === 'original' ? 'オリジナル' : 'アドバンス'}</p>
