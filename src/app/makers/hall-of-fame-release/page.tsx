...
export default async function Page() {
  ...
  return <main className="min-h-screen bg-slate-50 px-3 py-5"><div className="mx-auto max-w-7xl"><h1 className="text-2xl font-black">殿堂解除選手権</h1><p className="mt-1 text-sm text-gray-600">次に殿堂解除されると思うカードを選ぼう！</p>{!projectReady && <p className="mt-3 rounded border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900">Preview確認中：回答登録・集計は共通カードDBとの同期migration適用後に有効になります（未同期 {missing.length}枚）。</p>}<HallReleaseMaker cards={cards} draft={draft} canSave={projectReady} saved={saved} aggregates={aggregates} /></div></main>
}
