...
export default async function MakerSubmissionDetailPage({ params }: { params: Promise<{ slug: string; submissionId: string }> }) {
  ...
  const shareUrl = `https://twitter.com/intent/tweet?${new URLSearchParams({ text: `${prediction ? '殿堂解除予想' : submission.title}\n${project.title}`, url })}`
  return <main className="min-h-screen bg-slate-50 px-3 py-6"><article className="mx-auto max-w-5xl">
    ...
    <h1 className="mt-5 break-words text-2xl font-black">{prediction ? '殿堂解除予想' : submission.title}</h1>
    ...
    <div className="mt-5"><MakerSubmissionBoard submission={submission} groups={config.groups} enableActions exportTitle={prediction ? '2026年7月27日 殿堂解除選手権' : `${submission.title}｜${project.title}`} shareUrl={shareUrl} /></div>
