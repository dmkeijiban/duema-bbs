...
export default async function EditMakerSubmissionPage({ params }: { params: Promise<{ slug: string; submissionId: string }> }) {
  ...
  const { data: submission } = await admin.from('maker_submissions').select('id,title,comment,user_id').eq('id', submissionId).eq('project_id', project.id).eq('is_valid', true).eq('is_public', true).maybeSingle()
  if (!submission) notFound()
  if (submission.user_id !== null && submission.user_id !== user?.id) notFound()
