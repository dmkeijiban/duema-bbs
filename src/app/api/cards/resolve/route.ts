...
type Row = { id: string; name: string; name_kana: string | null; image_url: string | null; card_printings?: Printing[] }
...
function mapCard(row: Row, printing?: Printing): DeckCard {
  ...
  return { id: row.id, printingId: selected?.id ?? null, name: row.name, nameKana: row.name_kana, imageUrl: selected?.image_url ?? row.image_url, officialPageUrl: selected?.official_page_url ?? null, sourceKey: selected?.source_key ?? null }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [{ data: rows, error: cardError }, { data: aliases, error: aliasError }] = await Promise.all([
      admin.from('cards').select('id,name,name_kana,image_url,card_printings(id,source_key,official_page_url,image_url,is_representative,is_search_visible)').in('id', ids).eq('is_active', true),
    ...
    const aliasRowsResult = aliasCardIds.length
      ? await admin.from('cards').select('id,name,name_kana,image_url,card_printings(id,source_key,official_page_url,image_url,is_representative,is_search_visible)').in('id', aliasCardIds).eq('is_active', true)
      : { data: [], error: null }
