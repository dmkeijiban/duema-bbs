...
type Printing = { source_key: string; official_page_url: string | null; image_url: string | null; is_representative: boolean; is_search_visible: boolean }
...
function mapCard(row: Row, printing?: Printing): DeckCard {
  ...
  return { id: row.id, name: row.name, nameKana: row.name_kana, imageUrl: selected?.image_url ?? row.image_url, officialPageUrl: selected?.official_page_url ?? null, sourceKey: selected?.source_key ?? null }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [{ data: rows, error: cardError }, { data: aliases, error: aliasError }] = await Promise.all([
      admin.from('cards').select('id,name,name_kana,image_url,card_printings(source_key,official_page_url,image_url,is_representative,is_search_visible)').in('id', ids).eq('is_active', true),
