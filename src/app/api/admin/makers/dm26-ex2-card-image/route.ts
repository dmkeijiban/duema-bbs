...
export async function GET(request: NextRequest) {
  if (!verifyAdminCookie((await cookies()).get(ADMIN_COOKIE)?.value)) return new NextResponse('Unauthorized', { status: 401 })
  const rawUrl = request.nextUrl.searchParams.get('url')
  if (!rawUrl) return new NextResponse('Missing URL', { status: 400 })
  let url: URL
  try { url = new URL(rawUrl) } catch { return new NextResponse('Invalid URL', { status: 400 }) }
  if (url.protocol !== 'https:' || url.hostname !== 'dm.takaratomy.co.jp') return new NextResponse('Forbidden host', { status: 403 })
