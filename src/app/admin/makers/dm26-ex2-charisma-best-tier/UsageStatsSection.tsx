...
export default function UsageStatsSection({ stats, errorMessage }: Props) {
  ...
  return <section className="mt-4 rounded-xl border bg-slate-100/60 p-4">
    ...
    <Group title="回答状況">
      <Card label="回答登録者数" value={`${stats.registrantCount}人`} note="ログイン済みユニークユーザーのみ" /><Card label="回答登録件数" value={`${stats.submissionCount}件`} note="匿名と同一ユーザーの複数作品を含む" /><Card label="匿名登録件数" value={`${stats.anonymousSubmissionCount}件`} />
