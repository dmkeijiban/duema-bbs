...
export default function TierMaker({ cards, groups, initialDraft, unrated, canSave }: TierMakerProps) {
  ...
  const costOptions = [...new Set(cards.map(card => card.cost).filter((value): value is number => value !== null))].sort(
    (a, b) => a - b,
  )
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-3">
        {groups.map(group => (
          <div
            key={group.key}
            className={`grid min-h-28 grid-cols-[52px_1fr] rounded-xl border ${group.color}`}
          >
                  ...
                  <div key={cardId} className="group relative">
                    ...
                    <div className="mt-1 flex justify-center gap-1">
                      <button
                        type="button"
                        aria-label="左へ"
                        onClick={() => reorderCard(group.key, cardId, -1)}
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={() => {
              if (confirm('全てリセットしますか？')) setDraft(emptyMakerDraft(groups))
            }}
      ...
      <aside className="h-fit rounded-xl border bg-white p-3 lg:sticky lg:top-3">
        ...
        {cards.length === 0 && (
          <p className="py-10 text-center text-xs text-gray-400">企画カードは未登録です</p>
        )}
      </aside>
      ...
      {selected && (
        ...
        >
          <div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-2xl bg-white p-5">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              {groups.map(group => (
                <button
                  type="button"
                  key={group.key}
                  onClick={() => moveCard(selected.id, group.key)}
                  className={`rounded border p-3 font-black ${group.color}`}
                >
                  {group.label}
                </button>
              ))}
              {unrated && (
                <button
                  type="button"
                  onClick={() => moveCard(selected.id, null)}
            ...
            <button type="button" onClick={() => setSelected(null)} className="mt-3 w-full text-sm">
              閉じる
            </button>
          </div>
        </div>
