...
export default async function Page() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-3 py-5">
      ...
      <form action={updateGoodlifeAdSettingsAction} className="mt-5 space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        <p className="text-xs text-gray-500">許可済みの固定広告タグだけを読み込みます。広告配信コード自体は変更しません。</p>
        {([
          ['goodlife_inline_enabled', 'Goodlifeインライン広告', ads.enabled],
          ['goodlife_inline_thread_list', 'スレッド一覧・1件目の上', ads.threadList],
          ['goodlife_inline_thread_detail', 'スレッド詳細に表示', ads.threadDetail],
          ['goodlife_inline_footer', 'フッターの上に表示', ads.footer],
          ['goodlife_inline_desktop', 'PCで表示', ads.desktop],
          ['goodlife_inline_mobile', 'スマホで表示', ads.mobile],
        ...
        <button className="rounded bg-blue-600 px-4 py-2 text-xs font-bold text-white">広告設定を保存</button>
      </form>
    </main>
  )
}
