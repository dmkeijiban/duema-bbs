-- 管理CookieをServer Actionで検証した管理者が、任意のmaker投稿を更新・削除するためのservice role限定RPC。

begin;

create or replace function public.update_admin_maker_submission(
  p_project_id uuid,
...
begin
  ...
  if not exists(
    ...
    for update
  ) then raise exception 'MAKER_SUBMISSION_FORBIDDEN'; end if;
...
revoke all on function public.update_admin_maker_submission(uuid,uuid,text,text,jsonb) from public,anon,authenticated;
grant execute on function public.update_admin_maker_submission(uuid,uuid,text,text,jsonb) to service_role;

create or replace function public.delete_admin_maker_submission(
  p_project_id uuid,
  p_submission_id uuid
) returns void
language plpgsql
...
begin
  if not exists(
    ...
    where id=p_project_id and is_public and status='published'
  ) then raise exception 'MAKER_PROJECT_NOT_PUBLISHED'; end if;
...
revoke all on function public.delete_admin_maker_submission(uuid,uuid) from public,anon,authenticated;
grant execute on function public.delete_admin_maker_submission(uuid,uuid) to service_role;

insert into supabase_migrations.schema_migrations(version,name)
values('20260714210000','admin_maker_submission_mutations')
